...
export default function Navbar() {
  ...
  const headerBg = mounted && scrolled
    ? "bg-white/80 dark:bg-slate-900/80 backdrop-blur-[10px] border-b border-slate-100 dark:border-slate-800 shadow-sm"
    : "bg-transparent";
  ...
  return (
    <>
      ...
      >
        <div className={`max-w-[1290px] mx-auto px-4 sm:px-6 md:px-10 flex items-center justify-between transition-all duration-500 ${mounted && scrolled ? "h-[56px]" : "h-[68px]"}`}>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`relative px-4 py-2 text-[13px] tracking-[0.05em] rounded-lg transition-all duration-200 ${
                    link.branded
                      ? "normal-case"
                      : "uppercase"
                  } ${
